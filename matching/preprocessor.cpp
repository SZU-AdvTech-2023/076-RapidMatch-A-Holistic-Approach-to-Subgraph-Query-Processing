...
preprocessor::execute(const Graph *query_graph, const Graph *data_graph, catalog *storage, bool enable_elimination) {
    ...
    filter_time_ = 0;
#ifndef HOMOMORPHISM
    std::cout<<"after nlf_filter: storae->edge_relation[0][1].size_:"<<storage->edge_relations_[0][1].size_<<std::endl;
...
//------------------------------修改部分------------------建议建立R(u)-----------------// 
	    std::cout<<u<<" "<<v<<"-----------\n";
            string u_label = query_graph_->getVertexLabel(u);
            string v_label = query_graph_->getVertexLabel(v);
            scan_operator->execute(u_label, v_label, &(storage->edge_relations_[u][v]), true);
            std::cout<<u<<" "<<v<<storage->edge_relations_[u][v].size_<<"-----------\n";
        }
    }
