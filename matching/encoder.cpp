...
void encoder::convert_to_encoded_relation(catalog *storage, uint32_t u, uint32_t v) {
    ...
    for (uint32_t i = 0; i < u_candidates_cnt; ++i) {
        ...
        while (edge_index < edge_cnt) {// 
            ...
            //datagraph:u0-v0
            if (u0 == u_candidate) {//u0\CAǸ\C3i\BAź\F2ѡƥ\C5\E4\B5\E3 
                if (temp_buffer1_[v0] > 0) {//v0Ҳ\CA\C7v\B5ĺ\F2ѡƥ\C5\E4\B5\E3 
                    target_encoded_trie_relation.children_[offset + local_degree] = temp_buffer1_[v0] - 1;//v0\D4\DAv_candidates\B5\C4\D0\F2\BA\C5 
                    local_degree += 1;//һ\B8\F6while\C0\EF\C3\E6\B3\F6\CF\D6\C1\BD\B8\F6ƥ\C5\E4\B5Ĵ\CE\CA\FD 
                }
            }
