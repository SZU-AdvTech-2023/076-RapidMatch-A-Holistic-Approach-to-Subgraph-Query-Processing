...
//        total += labels_frequency_[labels_[i]];//------------wrong
//    }
//
...
void Graph::BuildNLF() {
    nlf_ = new sparse_hash_map<std::string, uint32_t>[vertices_count_];
    for (ui i = 0; i < vertices_count_; ++i) {
        ui count;
        ...
        for (ui j = 0; j < count; ++j) {
            ...
            std::string label = getVertexLabel(u);//------------------------------------------------------------ 
            nlf_[i][label] += 1;
        }
    }
}
...
void Graph::loadGraphFromFile(const std::string &file_path) {
    ...
    while (infile >> type) {
        if (type == 'v') { // Read vertex.
            ...
            if (labels_frequency_.find(label) == labels_frequency_.end()) {
                ...
                if (label.size() > max_label_id.size())//----------------------------------------------------------?
                    max_label_id = label;
            }
    ...
    infile.close();
    labels_count_ = (ui)labels_frequency_.size();//----------------------------------------------------------------
//	(ui)labels_frequency_.size() > (max_label_id + 1) ? (ui)labels_frequency_.size() : max_label_id + 1;
    //������ǩ����Ƶ�� 
...
void Graph::storeComparessedGraph(const std::string& degree_path, const std::string& edge_path,
    ...
    //label_outputfile.write(reinterpret_cast< char *>(labels_), label_array_bytes);
//---------------------------------------------------------------
...
void Graph::buildEdgeIndex() {
	std::cout<<"edge\n";
    edge_index_ = new sparse_hash_map<std::string, std::vector<edge>*>();

   edge cur_edge;
   for (uint32_t u = 0; u < vertices_count_; ++u) {
       std::string u_l = getVertexLabel(u);
       ...
       for (uint32_t i = 0; i < u_nbrs_cnt; ++i) {
           uint32_t v = u_nbrs[i];
           std::string v_l = getVertexLabel(v);
           std::string key = u_l+v_l;
