...
void GraphOperations::getKCore(const Graph *graph, int *core_table) {
    ...
    for (int i = 0; i < vertices_count; ++i) {
        int degree = graph->getVertexDegree(i);
        position[i] = offset[degree];//小于degree度的点的数量 
        vertices[position[i]] = i; //i按度数排序的位置 
        offset[degree] += 1;// 下一个degree度数的点的位置+1 
    }
    ...
    for (int i = 0; i < vertices_count; ++i) {
        ...
        for(int j = 0; j < static_cast<int>(count); ++j) {
            ...
            if (core_table[u] > core_table[v]) {
                ...
                int position_u = position[u];//小于点u的度数的点数量 
                int position_w = offset[cur_degree_u]; //小于等于 
...
void GraphOperations::compute_degeneracy_order(const Graph *graph, uint32_t *degeneracy_order) {
	...
	//degree累加 offset[i]=num of vertices which degree <=i;
    int start = 0;
    for (int i = 0; i < max_degree + 1; ++i) {
        offset[i] = start;
        start += degree_bin[i];
