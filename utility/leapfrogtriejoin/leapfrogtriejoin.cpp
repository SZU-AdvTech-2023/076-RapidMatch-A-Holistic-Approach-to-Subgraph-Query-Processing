        ...
        if (start_depth >= num_core_vertex_) {
            ...
            while (true) {
                while (idx_[cur_depth] < num_local_candidates_[cur_depth]) {
                	//idx_[cur_depth]是索引local_candidates_[cur_depth]的变量~i 
...
//---------------------------------------------------------------------------------------------------------------------------
                        ...
                        if (count_ >= output_count_limit_) {// no need 
                            return count_;
                        }
                    }//-------------------------------------------------------------------
...
#if RELATION_STRUCTURE == 0
        uint32_t v = si_buffer_[idx_[start_depth]];//候选点 
#elif RELATION_STRUCTURE == 1 || RELATION_STRUCTURE == 2
...
//333--------------------------------------------------------------------------------
        ...
        else {// cur_depth不是最后一个 
//444---------------------------------------------------------------------------------------
            idx_[cur_depth] = 0;
            while (true) {
                while (idx_[cur_depth] < num_local_candidates_[cur_depth]) {
                    v = local_candidates_[cur_depth][idx_[cur_depth]++];
					//和之前不一样，local_candidates_[cur_depth]经过一个函数处理 
#ifndef HOMOMORPHISM
                    if (visited_[v]) {
#ifdef COLLECT_FAIL_SI_STATISTICS
                        iso_conflict_count_ += 1;
    ...
    }//for(idx_[start_depth]:0-num_local_candidates[])
